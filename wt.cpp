...
wavelet_L::wavelet_L(string bwt_, sd_vector<> a_, sd_vector<> c_, sd_vector<> g_, sd_vector<> t_):
	bwt(bwt_), a(a_), c(c_), g(g_), t(t_)
{
...
int wavelet_L::rank(int i, char X){
	map<char, sd_vector<>::rank_1_type> bp_rank = {{'A',a_rank},{'C',c_rank},{'G',g_rank},{'T',t_rank}};
	if (X - 'A' < 27) {
...
float wavelet_L::save(string file){
	...
	if (fb.open("data/bwt"+file, ios::out)) {
		...
		mb_size += size_in_mega_bytes(a)+size_in_mega_bytes(c)+size_in_mega_bytes(g)+size_in_mega_bytes(t);
